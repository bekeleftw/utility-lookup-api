...
# Load ZIP-level electric corrections (user feedback)
_electric_zip_corrections = None
def load_electric_zip_corrections():
    global _electric_zip_corrections
    if _electric_zip_corrections is None:
        corrections_file = Path(__file__).parent / 'data' / 'electric_zip_corrections.json'
        if corrections_file.exists():
            with open(corrections_file, 'r') as f:
                data = json.load(f)
                _electric_zip_corrections = data.get('corrections', {})
        else:
            _electric_zip_corrections = {}
    return _electric_zip_corrections

def get_electric_zip_correction(zip_code: str) -> Optional[Dict]:
    """Check if there's a user-feedback correction for this ZIP."""
    corrections = load_electric_zip_corrections()
...
def lookup_electric_only(lat: float, lon: float, city: str, county: str, state: str, zip_code: str, address: str = None, use_pipeline: bool = True) -> Optional[Dict]:
    ...
    try:
        # Priority -1: Check user-feedback ZIP corrections first (highest priority)
        zip_correction = get_electric_zip_correction(zip_code)
        if zip_correction:
            zip_correction['CITY'] = city
            return _add_deregulated_info(zip_correction, state, zip_code)
