...
let otherSection = '';
if (util.other_providers && util.other_providers.length > 0) { otherSection = '<div class="up-other-providers"><div class="up-other-label up-other-toggle" data-card="' + cardId + '">Other possible providers <span class="up-other-caret">▼</span></div><div class="up-other-list up-other-collapsed" id="' + cardId + '-others">' + util.other_providers.map(function(p, i) { const altId = cardId + '-alt-' + i; let altDetails = ''; if (p.phone && p.phone !== 'NOT AVAILABLE') { altDetails += '<div class="up-alt-detail"><span class="up-alt-label">Phone</span><a href="tel:' + p.phone + '">' + p.phone + '</a></div>'; } if (p.website && p.website !== 'NOT AVAILABLE') { const wurl = p.website.startsWith('http') ? p.website : 'https://' + p.website; altDetails += '<div class="up-alt-detail"><span class="up-alt-label">Website</span><a href="' + wurl + '" target="_blank">Visit site</a></div>'; } return '<div class="up-alt-card" id="' + altId + '"><div class="up-alt-header"><div class="up-alt-name"><span class="up-alt-icon">' + icons[type] + '</span>' + p.name + '</div><div class="up-alt-feedback"><span>Correct?</span><button class="up-alt-btn yes" data-alt-id="' + altId + '" data-provider="' + p.name + '" data-type="' + type + '" data-response="yes">Yes</button><button class="up-alt-btn no" data-alt-id="' + altId + '" data-provider="' + p.name + '" data-type="' + type + '" data-response="no">No</button><span class="up-alt-done" id="' + altId + '-done">Thanks!</span></div></div>' + (altDetails ? '<div class="up-alt-details">' + altDetails + '</div>' : '') + '</div>'; }).join('') + '</div></div>'; }
