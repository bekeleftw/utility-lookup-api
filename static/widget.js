...
let otherSection = '';
if (util.other_providers && util.other_providers.length > 0) { otherSection = '<div class="up-other-providers"><div class="up-other-label up-other-toggle" data-card="' + cardId + '">Other possible providers <span class="up-other-caret">▼</span></div><div class="up-other-list up-other-collapsed" id="' + cardId + '-others">' + util.other_providers.map(function(p, i) { const altId = cardId + '-alt-' + i; let altDetails = ''; if (p.phone && p.phone !== 'NOT AVAILABLE') { altDetails += '<div class="up-detail"><span class="up-detail-label">Phone</span><span class="up-detail-value"><a href="tel:' + p.phone + '">' + p.phone + '</a></span></div>'; } if (p.website && p.website !== 'NOT AVAILABLE') { const wurl = p.website.startsWith('http') ? p.website : 'https://' + p.website; altDetails += '<div class="up-detail"><span class="up-detail-label">Website</span><span class="up-detail-value"><a href="' + wurl + '" target="_blank">Visit site</a></span></div>'; } return '<div style="padding:16px 0;border-bottom:1px dashed #e5e7eb;" id="' + altId + '"><div class="up-provider-name" style="font-size:16px;margin-bottom:8px;">' + p.name + '</div><div class="up-details"><div class="up-details-left">' + altDetails + '</div><div class="up-feedback-inline"><span>Correct?</span><button class="up-alt-btn yes" data-alt-id="' + altId + '" data-provider="' + p.name + '" data-type="' + type + '" data-response="yes">Yes</button><button class="up-alt-btn no" data-alt-id="' + altId + '" data-provider="' + p.name + '" data-type="' + type + '" data-response="no">No</button><span class="up-alt-done" id="' + altId + '-done">Thanks!</span></div></div></div>'; }).join('') + '</div></div>'; }
